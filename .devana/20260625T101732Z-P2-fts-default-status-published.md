...
DEVANA-SUMMARY: Status=fixed | P2 high src/fts.ts:67-72,89 - Published default is intentional and consistent with EmDash search (which also defaults status to published); documented the behavior in JSDoc + README and added a test, rather than introducing a divergence by omitting the clause.