...
- 2026-06-27: validation review reopened. The documentation and clearer error improve the contract, but the fixed-state bar is not met because the original counterexample is still reachable: `compileStructuralFilters([{ path: "$.a[*].b[*]", op: "exists" }])` still throws instead of compiling or being rejected by shared validation. The original finding remains open unless the compiler supports multi-wildcard paths, validation rejects them for the compiler-specific entry point, or this limitation is explicitly accepted as `wontfix`.
- 2026-06-27: fixed by compiler support. The structural SQL compiler now keeps the first-wildcard `json_each` grouping and compiles deeper wildcards to nested array-guarded `json_each` joins, so `compileStructuralFilters([{ path: "$.a[*].b[*]", op: "eq", value: "z" }])` returns an executable plan instead of throwing. Added regressions for the original multi-wildcard shape and for same-first-wildcard grouping with a nested wildcard filter.
- 2026-06-27: review hardening. Nested wildcards now compile to visible nested `json_each` joins with `json_type(...)= 'array'` guards at each wildcard step, so object/scalar values do not satisfy wildcard traversal. Nested `matchedPathExpressions` now use the nested alias chain to produce concrete leaf paths such as `$.a[1].b[0]`.

DEVANA-KEY: src/schema.ts:6,26; src/structural.ts:140,261 | P2 | nested-wildcard-structural-throw
DEVANA-SUMMARY: Status=fixed | P2 medium src/schema.ts:6,26; src/structural.ts:140,261 - compileStructuralFilters now supports schema-valid multi-wildcard paths with array-guarded nested json_each joins and concrete nested matchedPathExpressions instead of throwing.
